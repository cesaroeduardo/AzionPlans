import SectionBanner from "../components/SectionBanner";
import SectionSignup from "../components/SectionSignup";
import SectionTalkexpert from "../components/SectionTalkexpert";
import SectionPrincing from "../components/SectionPrincing";
import "./PlansPage.css";
const PlansPage = () => {
  return (
    <div className="plans-page">
      <SectionBanner
        title="How do you pay for Azion?"
        description="Our usage-based pricing model gives you the freedom to customize your choice for more efficiency paying for what you use"
        tagline="pricing model"
      />
      <SectionSignup />
      <SectionTalkexpert />
      <SectionPrincing />
      <div
        className="section-features"
        data-scroll-to="sectionFeaturesContainer"
      >
        <div className="plan-headers">
          <div className="headers">
            <div className="first1">
              <div className="features">Features</div>
            </div>
            <div className="planheader">
              <div className="row90">
                <div className="name">Developer</div>
                <div className="tag8">
                  <div className="tagmain">
                    <b className="label12">Recommended</b>
                    <div className="external-link">
                      <img
                        className="infotooltip-main"
                        alt=""
                        src="/icon74.svg"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="starting-at">starting at</div>
              <div className="price1">
                <div className="free">Free</div>
              </div>
              <div className="for-testing-purposes">
                For testing purposes and are individuals without professional
                needs.
              </div>
              <div className="button12">
                <div className="label13">get started</div>
                <img className="icon106" alt="" />
              </div>
              <div className="button13">
                <div className="label13">Label</div>
                <img className="icon106" alt="" src="/icon8.svg" />
              </div>
            </div>
            <div className="planheader">
              <div className="row90">
                <div className="name">Business</div>
                <div className="tag8">
                  <div className="tagmain">
                    <b className="label12">Recommended</b>
                    <div className="external-link">
                      <img
                        className="infotooltip-main"
                        alt=""
                        src="/icon74.svg"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="starting-at">starting at</div>
              <div className="price2">
                <div className="free">$250</div>
                <div className="usdmonth">USD/month</div>
              </div>
              <div className="for-testing-purposes">
                <p className="customers-with-a">
                  Customers with a business profile are small growing
                  businesses.
                </p>
              </div>
              <div className="button14">
                <div className="label13">get business</div>
                <img className="icon106" alt="" src="/icon91.svg" />
              </div>
              <div className="button13">
                <div className="label13">contact us</div>
                <img className="icon106" alt="" src="/icon91.svg" />
              </div>
            </div>
            <div className="planheader">
              <div className="row90">
                <div className="name">Enterprise</div>
                <div className="tag8">
                  <div className="tagmain">
                    <b className="label12">Recommended</b>
                    <div className="external-link">
                      <img
                        className="infotooltip-main"
                        alt=""
                        src="/icon74.svg"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="starting-at">starting at</div>
              <div className="price3">
                <div className="free">USD 5,000.00</div>
                <div className="usdmonth1">USD/month</div>
              </div>
              <div className="for-testing-purposes">
                For companies that use Azion for production and need help to get
                the most out of the platform.
              </div>
              <div className="button14">
                <div className="label13">get enterprise</div>
                <img className="icon106" alt="" src="/icon8.svg" />
              </div>
              <div className="button13">
                <div className="label13">contact us</div>
                <img className="icon106" alt="" src="/icon8.svg" />
              </div>
            </div>
            <div className="planheader">
              <div className="row90">
                <div className="name">Mission Critical</div>
                <div className="tag8">
                  <div className="tagmain">
                    <b className="label12">Recommended</b>
                    <div className="external-link">
                      <img
                        className="infotooltip-main"
                        alt=""
                        src="/icon74.svg"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="starting-at">starting at</div>
              <div className="price3">
                <div className="free">USD 18,750.00</div>
                <div className="usdmonth2">USD/month</div>
              </div>
              <div className="for-testing-purposes">
                Service performance, security, availability and scalability.
              </div>
              <div className="button14">
                <div className="label13">get mission critical</div>
                <img className="icon106" alt="" src="/icon8.svg" />
              </div>
              <div className="button13">
                <div className="label13">contact us</div>
                <img className="icon106" alt="" src="/icon8.svg" />
              </div>
            </div>
          </div>
        </div>
        <div className="plan-headers-responsive">
          <div className="chips">
            <div className="chips1">Developer</div>
          </div>
          <div className="chips2">
            <div className="chips1">Business</div>
          </div>
          <div className="chips2">
            <div className="chips1">Enterprise</div>
          </div>
          <div className="chips2">
            <div className="chips1">Mission Critical</div>
          </div>
        </div>
        <div className="content12">
          <div className="table14">
            <div className="tableheader57">
              <div className="header58">Product Features</div>
            </div>
            <div className="featurerow">
              <div className="featurecell">
                <div className="content13">
                  <div className="feature-name">
                    Access to all Azion products
                  </div>
                  <div className="description67">
                    Developers will have access to all Azion products and pay by
                    use.
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon101.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon111.svg" />
                  </div>
                </div>
              </div>
              <div className="cell298">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon101.svg" />
                  </div>
                </div>
              </div>
              <div className="cell299">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon101.svg" />
                  </div>
                </div>
              </div>
            </div>
            <div className="featurerow1">
              <div className="featurecell">
                <div className="content13">
                  <div className="feature-name">Beta Access</div>
                  <div className="description67">
                    Azion might give early access to new features for customers
                    under Developer Plan that are still under refinement by our
                    product teams. It’s important to note that these features
                    are not covered by a Service Level Agreement.
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon131.svg" />
                  </div>
                </div>
              </div>
              <div className="cell298">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell299">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon151.svg" />
                  </div>
                </div>
              </div>
            </div>
            <div className="featurerow2">
              <div className="featurecell2">
                <div className="content13">
                  <div className="feature-name">Uptime SLA</div>
                  <div className="description67">Description</div>
                </div>
              </div>
              <div className="cell304">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell304">
                <div className="text350">100%</div>
              </div>
              <div className="cell304">
                <div className="text350">100%</div>
              </div>
              <div className="cell304">
                <div className="text350">100%</div>
              </div>
            </div>
            <div className="featurerow3">
              <div className="featurecell3">
                <div className="content16">
                  <div className="feature-name3">Unmetered DDoS Protection</div>
                  <div className="description67">
                    We will give our customers unmetered mitigation during a
                    DDoS incident.
                  </div>
                </div>
                <div className="infotooltip">
                  <div className="infotooltip-main">
                    <div className="icon135">
                      <img className="icon136" alt="" src="/icon20.svg" />
                    </div>
                    <div className="tooltip">
                      <div className="arrow">
                        <img
                          className="arrowpath-icon"
                          alt=""
                          src="/arrowpath.svg"
                        />
                      </div>
                      <div className="column">
                        <div className="arrow1">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath1.svg"
                          />
                        </div>
                        <div className="tooltip1">
                          <b className="tooltip-title">
                            Unmetered DDoS Protection
                          </b>
                          <div className="text353">
                            Customers who subscribed to any plan will have
                            access to DDoS protection without the risk of higher
                            charges.
                          </div>
                        </div>
                        <div className="arrow1">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath2.svg"
                          />
                        </div>
                      </div>
                      <div className="arrow3">
                        <img
                          className="arrowpath-icon"
                          alt=""
                          src="/arrowpath3.svg"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon171.svg" />
                  </div>
                </div>
              </div>
              <div className="cell298">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
              <div className="cell299">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
            </div>
            <div className="featurerow4">
              <div className="featurecell">
                <div className="content13">
                  <div className="feature-name">Single Sign On (SSO)</div>
                  <div className="description67">Federated authentication</div>
                </div>
              </div>
              <div className="cell296">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon131.svg" />
                  </div>
                </div>
              </div>
              <div className="cell298">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
              <div className="cell299">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
            </div>
            <div className="featurerow5">
              <div className="featurecell5">
                <div className="content13">
                  <div className="feature-name">Edge Network Custom Maps</div>
                  <div className="description67">Description</div>
                </div>
              </div>
              <div className="cell304">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell304">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon131.svg" />
                  </div>
                </div>
              </div>
              <div className="cell318">
                <div className="tag12">
                  <div className="tag13">Paid Add-on</div>
                </div>
                <div className="upgrade-using-add">Upgrade using Add On</div>
              </div>
              <div className="cell304">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="table15">
            <div className="tableheader58">
              <div className="header58">Services and Support</div>
            </div>
            <div className="featurerow6">
              <div className="featurecell6">
                <div className="content13">
                  <div className="feature-name">Tech Support Options</div>
                  <div className="description67">
                    Can open tickets via ticket and email, reporting bugs and
                    incidents.
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="text350">Tickets and e-mail</div>
              </div>
              <div className="cell298">
                <div className="text350">Tickets, e-mail and phone</div>
              </div>
              <div className="cell299">
                <div className="text350">Tickets, e-mail and phone</div>
              </div>
            </div>
            <div className="featurerow7">
              <div className="featurecell3">
                <div className="content16">
                  <div className="feature-name3">Techical Support SLA</div>
                  <div className="description67">
                    Azion might give early access to new features for customers
                    under Developer Plan that are still under refinement by our
                    product teams. It’s important to note that these features
                    are not covered by a Service Level Agreement.
                  </div>
                </div>
                <div className="infotooltip1">
                  <div className="infotooltip-main">
                    <div className="icon135">
                      <img className="icon136" alt="" src="/icon20.svg" />
                    </div>
                    <div className="tooltip">
                      <div className="arrow">
                        <img
                          className="arrowpath-icon"
                          alt=""
                          src="/arrowpath.svg"
                        />
                      </div>
                      <div className="column">
                        <div className="arrow1">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath1.svg"
                          />
                        </div>
                        <div className="tooltip1">
                          <b className="tooltip-title">Techical Support SLA</b>
                          <div className="text353">
                            24x7 access to Support Engineers via ticket, email
                            and phone
                          </div>
                        </div>
                        <div className="arrow1">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath2.svg"
                          />
                        </div>
                      </div>
                      <div className="arrow3">
                        <img
                          className="arrowpath-icon"
                          alt=""
                          src="/arrowpath3.svg"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon171.svg" />
                  </div>
                </div>
              </div>
              <div className="cell298">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
              <div className="cell299">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
            </div>
            <div className="featurerow8">
              <div className="featurecell3">
                <div className="content16">
                  <div className="feature-name3">Education Programs</div>
                  <div className="description67">
                    Unlimited access to documentation, guides, whitepapers and
                    bug reports.
                  </div>
                </div>
                <div className="infotooltip">
                  <div className="infotooltip-main">
                    <div className="icon135">
                      <img className="icon136" alt="" src="/icon20.svg" />
                    </div>
                    <div className="tooltip">
                      <div className="arrow">
                        <img
                          className="arrowpath-icon"
                          alt=""
                          src="/arrowpath.svg"
                        />
                      </div>
                      <div className="column">
                        <div className="arrow1">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath1.svg"
                          />
                        </div>
                        <div className="tooltip1">
                          <b className="tooltip-title">Education Programs</b>
                          <div className="text353">
                            Unlimited access to documentation, guides,
                            whitepapers and bug reports.
                          </div>
                        </div>
                        <div className="arrow1">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath2.svg"
                          />
                        </div>
                      </div>
                      <div className="arrow3">
                        <img
                          className="arrowpath-icon"
                          alt=""
                          src="/arrowpath3.svg"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon171.svg" />
                  </div>
                </div>
              </div>
              <div className="cell298">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
              <div className="cell299">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
            </div>
            <div className="featurerow9">
              <div className="featurecell9">
                <div className="content13">
                  <div className="feature-name">Custom Education Programs</div>
                  <div className="description67">Description</div>
                </div>
              </div>
              <div className="cell296">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon131.svg" />
                  </div>
                </div>
              </div>
              <div className="cell334">
                <div className="tag12">
                  <div className="tag13">Paid Add-on</div>
                </div>
                <div className="upgrade-using-add">Paid Add-on</div>
              </div>
              <div className="cell335">
                <div className="tag12">
                  <div className="tag13">Paid Add-on</div>
                </div>
                <div className="upgrade-using-add">Paid Add-on</div>
              </div>
            </div>
            <div className="featurerow10">
              <div className="featurecell10">
                <div className="content16">
                  <div className="feature-name3">Integration Services</div>
                  <div className="description67">Description</div>
                </div>
                <div className="infotooltip3">
                  <div className="infotooltip-main">
                    <div className="icon135">
                      <img className="icon136" alt="" src="/icon20.svg" />
                    </div>
                    <div className="tooltip">
                      <div className="arrow12">
                        <img className="arrow-icon" alt="" src="/arrow.svg" />
                        <img className="stroke-icon" alt="" src="/stroke.svg" />
                      </div>
                      <div className="column3">
                        <div className="arrow13">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath1.svg"
                          />
                        </div>
                        <div className="tooltip7">
                          <div className="content24">
                            <div className="text359">
                              <b className="tooltip-title3">Tooltip Title</b>
                              <div className="text360">
                                <p className="customers-with-a">{`Nam dapibus nisl vitae elit fringilla rutrum. Aenean sollicitudin, erat a elementum rutrum, neque sem pretium metus, quis mollis nisl nunc et massa. Vestibulum sed metus in lorem tristique ullamcorper id vitae erat. `}</p>
                                <p className="customers-with-a">&nbsp;</p>
                                <p className="customers-with-a">
                                  Nulla mollis sapien sollicitudin lacinia
                                  lacinia.
                                </p>
                              </div>
                            </div>
                            <div className="iconbutton">
                              <div className="iconbox">
                                <div className="icon187">
                                  <img
                                    className="icon188"
                                    alt=""
                                    src="/icon181.svg"
                                  />
                                </div>
                              </div>
                            </div>
                          </div>
                          <div className="slot2">
                            <div className="slot3">Slot</div>
                          </div>
                        </div>
                        <div className="arrow14">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrow1.svg"
                          />
                          <img
                            className="stroke-icon1"
                            alt=""
                            src="/stroke1.svg"
                          />
                        </div>
                      </div>
                      <div className="arrow15">
                        <img className="arrow-icon" alt="" src="/arrow2.svg" />
                        <img
                          className="stroke-icon"
                          alt=""
                          src="/stroke2.svg"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="text350">1 hour / year included</div>
              </div>
              <div className="cell298">
                <div className="text350">20 hours / year included</div>
              </div>
              <div className="cell299">
                <div className="text350">40 hours / year included</div>
              </div>
            </div>
            <div className="featurerow11">
              <div className="featurecell10">
                <div className="content16">
                  <div className="feature-name3">Best Practices Review</div>
                  <div className="description67">Description</div>
                </div>
                <div className="infotooltip3">
                  <div className="infotooltip-main">
                    <div className="icon135">
                      <img className="icon136" alt="" src="/icon20.svg" />
                    </div>
                    <div className="tooltip">
                      <div className="arrow12">
                        <img className="arrow-icon" alt="" src="/arrow.svg" />
                        <img className="stroke-icon" alt="" src="/stroke.svg" />
                      </div>
                      <div className="column3">
                        <div className="arrow13">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath1.svg"
                          />
                        </div>
                        <div className="tooltip7">
                          <div className="content24">
                            <div className="text359">
                              <b className="tooltip-title3">Tooltip Title</b>
                              <div className="text360">
                                <p className="customers-with-a">{`Nam dapibus nisl vitae elit fringilla rutrum. Aenean sollicitudin, erat a elementum rutrum, neque sem pretium metus, quis mollis nisl nunc et massa. Vestibulum sed metus in lorem tristique ullamcorper id vitae erat. `}</p>
                                <p className="customers-with-a">&nbsp;</p>
                                <p className="customers-with-a">
                                  Nulla mollis sapien sollicitudin lacinia
                                  lacinia.
                                </p>
                              </div>
                            </div>
                            <div className="iconbutton">
                              <div className="iconbox">
                                <div className="icon187">
                                  <img
                                    className="icon188"
                                    alt=""
                                    src="/icon181.svg"
                                  />
                                </div>
                              </div>
                            </div>
                          </div>
                          <div className="slot2">
                            <div className="slot3">Slot</div>
                          </div>
                        </div>
                        <div className="arrow14">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrow1.svg"
                          />
                          <img
                            className="stroke-icon1"
                            alt=""
                            src="/stroke1.svg"
                          />
                        </div>
                      </div>
                      <div className="arrow15">
                        <img className="arrow-icon" alt="" src="/arrow2.svg" />
                        <img
                          className="stroke-icon"
                          alt=""
                          src="/stroke2.svg"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon131.svg" />
                  </div>
                </div>
              </div>
              <div className="cell298">
                <div className="text350">20 hours / year included</div>
              </div>
              <div className="cell299">
                <div className="text350">40 hours / year included</div>
              </div>
            </div>
            <div className="featurerow12">
              <div className="featurecell12">
                <div className="content16">
                  <div className="feature-name3">Named TAM</div>
                  <div className="description67">Description</div>
                </div>
                <div className="infotooltip3">
                  <div className="infotooltip-main">
                    <div className="icon135">
                      <img className="icon136" alt="" src="/icon20.svg" />
                    </div>
                    <div className="tooltip">
                      <div className="arrow12">
                        <img className="arrow-icon" alt="" src="/arrow.svg" />
                        <img className="stroke-icon" alt="" src="/stroke.svg" />
                      </div>
                      <div className="column3">
                        <div className="arrow13">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath1.svg"
                          />
                        </div>
                        <div className="tooltip7">
                          <div className="content24">
                            <div className="text359">
                              <b className="tooltip-title3">Tooltip Title</b>
                              <div className="text360">
                                <p className="customers-with-a">{`Nam dapibus nisl vitae elit fringilla rutrum. Aenean sollicitudin, erat a elementum rutrum, neque sem pretium metus, quis mollis nisl nunc et massa. Vestibulum sed metus in lorem tristique ullamcorper id vitae erat. `}</p>
                                <p className="customers-with-a">&nbsp;</p>
                                <p className="customers-with-a">
                                  Nulla mollis sapien sollicitudin lacinia
                                  lacinia.
                                </p>
                              </div>
                            </div>
                            <div className="iconbutton">
                              <div className="iconbox">
                                <div className="icon187">
                                  <img
                                    className="icon188"
                                    alt=""
                                    src="/icon181.svg"
                                  />
                                </div>
                              </div>
                            </div>
                          </div>
                          <div className="slot2">
                            <div className="slot3">Slot</div>
                          </div>
                        </div>
                        <div className="arrow14">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrow1.svg"
                          />
                          <img
                            className="stroke-icon1"
                            alt=""
                            src="/stroke1.svg"
                          />
                        </div>
                      </div>
                      <div className="arrow15">
                        <img className="arrow-icon" alt="" src="/arrow2.svg" />
                        <img
                          className="stroke-icon"
                          alt=""
                          src="/stroke2.svg"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon131.svg" />
                  </div>
                </div>
              </div>
              <div className="cell334">
                <div className="tag12">
                  <div className="tag13">Paid Add-on</div>
                </div>
                <div className="upgrade-using-add">Paid Add-on</div>
              </div>
              <div className="cell347">
                <div className="tag12">
                  <div className="tag13">Paid Add-on</div>
                </div>
                <div className="upgrade-using-add">Paid Add-on</div>
              </div>
            </div>
            <div className="featurerow13">
              <div className="featurecell3">
                <div className="content16">
                  <div className="feature-name3">
                    Named Customer Success Manager
                  </div>
                  <div className="description67">Description</div>
                </div>
                <div className="infotooltip3">
                  <div className="infotooltip-main">
                    <div className="icon135">
                      <img className="icon136" alt="" src="/icon20.svg" />
                    </div>
                    <div className="tooltip">
                      <div className="arrow12">
                        <img className="arrow-icon" alt="" src="/arrow.svg" />
                        <img className="stroke-icon" alt="" src="/stroke.svg" />
                      </div>
                      <div className="column3">
                        <div className="arrow13">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath1.svg"
                          />
                        </div>
                        <div className="tooltip7">
                          <div className="content24">
                            <div className="text359">
                              <b className="tooltip-title3">Tooltip Title</b>
                              <div className="text360">
                                <p className="customers-with-a">{`Nam dapibus nisl vitae elit fringilla rutrum. Aenean sollicitudin, erat a elementum rutrum, neque sem pretium metus, quis mollis nisl nunc et massa. Vestibulum sed metus in lorem tristique ullamcorper id vitae erat. `}</p>
                                <p className="customers-with-a">&nbsp;</p>
                                <p className="customers-with-a">
                                  Nulla mollis sapien sollicitudin lacinia
                                  lacinia.
                                </p>
                              </div>
                            </div>
                            <div className="iconbutton">
                              <div className="iconbox">
                                <div className="icon187">
                                  <img
                                    className="icon188"
                                    alt=""
                                    src="/icon19.svg"
                                  />
                                </div>
                              </div>
                            </div>
                          </div>
                          <div className="slot2">
                            <div className="slot3">Slot</div>
                          </div>
                        </div>
                        <div className="arrow14">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrow1.svg"
                          />
                          <img
                            className="stroke-icon1"
                            alt=""
                            src="/stroke1.svg"
                          />
                        </div>
                      </div>
                      <div className="arrow15">
                        <img className="arrow-icon" alt="" src="/arrow2.svg" />
                        <img
                          className="stroke-icon"
                          alt=""
                          src="/stroke2.svg"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon131.svg" />
                  </div>
                </div>
              </div>
              <div className="cell298">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
              <div className="cell299">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
            </div>
            <div className="featurerow4">
              <div className="featurecell14">
                <div className="content16">
                  <div className="feature-name3">Security Response Team</div>
                  <div className="description67">Description</div>
                </div>
                <div className="infotooltip3">
                  <div className="infotooltip-main">
                    <div className="icon135">
                      <img className="icon136" alt="" src="/icon20.svg" />
                    </div>
                    <div className="tooltip">
                      <div className="arrow12">
                        <img className="arrow-icon" alt="" src="/arrow.svg" />
                        <img className="stroke-icon" alt="" src="/stroke.svg" />
                      </div>
                      <div className="column3">
                        <div className="arrow13">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath1.svg"
                          />
                        </div>
                        <div className="tooltip7">
                          <div className="content24">
                            <div className="text359">
                              <b className="tooltip-title3">Tooltip Title</b>
                              <div className="text360">
                                <p className="customers-with-a">{`Nam dapibus nisl vitae elit fringilla rutrum. Aenean sollicitudin, erat a elementum rutrum, neque sem pretium metus, quis mollis nisl nunc et massa. Vestibulum sed metus in lorem tristique ullamcorper id vitae erat. `}</p>
                                <p className="customers-with-a">&nbsp;</p>
                                <p className="customers-with-a">
                                  Nulla mollis sapien sollicitudin lacinia
                                  lacinia.
                                </p>
                              </div>
                            </div>
                            <div className="iconbutton">
                              <div className="iconbox">
                                <div className="icon187">
                                  <img
                                    className="icon188"
                                    alt=""
                                    src="/icon181.svg"
                                  />
                                </div>
                              </div>
                            </div>
                          </div>
                          <div className="slot2">
                            <div className="slot3">Slot</div>
                          </div>
                        </div>
                        <div className="arrow14">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrow1.svg"
                          />
                          <img
                            className="stroke-icon1"
                            alt=""
                            src="/stroke1.svg"
                          />
                        </div>
                      </div>
                      <div className="arrow15">
                        <img className="arrow-icon" alt="" src="/arrow2.svg" />
                        <img
                          className="stroke-icon"
                          alt=""
                          src="/stroke2.svg"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon131.svg" />
                  </div>
                </div>
              </div>
              <div className="cell354">
                <div className="tag12">
                  <div className="tag13">Paid Add-on</div>
                </div>
                <div className="upgrade-using-add">Upgrade using Add On</div>
              </div>
              <div className="cell355">
                <div className="text350">20 hours / month included</div>
              </div>
            </div>
            <div className="featurerow15">
              <div className="featurecell12">
                <div className="content16">
                  <div className="feature-name3">Slack Channel</div>
                  <div className="description67">Description</div>
                </div>
                <div className="infotooltip3">
                  <div className="infotooltip-main">
                    <div className="icon135">
                      <img className="icon136" alt="" src="/icon20.svg" />
                    </div>
                    <div className="tooltip">
                      <div className="arrow12">
                        <img className="arrow-icon" alt="" src="/arrow.svg" />
                        <img className="stroke-icon" alt="" src="/stroke.svg" />
                      </div>
                      <div className="column3">
                        <div className="arrow13">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath1.svg"
                          />
                        </div>
                        <div className="tooltip7">
                          <div className="content24">
                            <div className="text359">
                              <b className="tooltip-title3">Tooltip Title</b>
                              <div className="text360">
                                <p className="customers-with-a">{`Nam dapibus nisl vitae elit fringilla rutrum. Aenean sollicitudin, erat a elementum rutrum, neque sem pretium metus, quis mollis nisl nunc et massa. Vestibulum sed metus in lorem tristique ullamcorper id vitae erat. `}</p>
                                <p className="customers-with-a">&nbsp;</p>
                                <p className="customers-with-a">
                                  Nulla mollis sapien sollicitudin lacinia
                                  lacinia.
                                </p>
                              </div>
                            </div>
                            <div className="iconbutton">
                              <div className="iconbox">
                                <div className="icon187">
                                  <img
                                    className="icon188"
                                    alt=""
                                    src="/icon181.svg"
                                  />
                                </div>
                              </div>
                            </div>
                          </div>
                          <div className="slot2">
                            <div className="slot3">Slot</div>
                          </div>
                        </div>
                        <div className="arrow14">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrow1.svg"
                          />
                          <img
                            className="stroke-icon1"
                            alt=""
                            src="/stroke3.svg"
                          />
                        </div>
                      </div>
                      <div className="arrow15">
                        <img className="arrow-icon" alt="" src="/arrow2.svg" />
                        <img
                          className="stroke-icon"
                          alt=""
                          src="/stroke2.svg"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon131.svg" />
                  </div>
                </div>
              </div>
              <div className="cell334">
                <div className="tag12">
                  <div className="tag13">Paid Add-on</div>
                </div>
                <div className="upgrade-using-add">Upgrade using Add On</div>
              </div>
              <div className="cell347">
                <div className="tag12">
                  <div className="tag13">Paid Add-on</div>
                </div>
                <div className="upgrade-using-add">Upgrade using Add On</div>
              </div>
            </div>
          </div>
          <div className="table14">
            <div className="tableheader59">
              <div className="header58">Compliance</div>
            </div>
            <div className="featurerow16">
              <div className="featurecell3">
                <div className="content16">
                  <div className="feature-name3">ISO 27001</div>
                  <div className="description67">
                    Compliance feature, information security certification.
                  </div>
                </div>
                <div className="infotooltip3">
                  <div className="infotooltip-main">
                    <div className="icon135">
                      <img className="icon136" alt="" src="/icon20.svg" />
                    </div>
                    <div className="tooltip">
                      <div className="arrow12">
                        <img className="arrow-icon" alt="" src="/arrow.svg" />
                        <img className="stroke-icon" alt="" src="/stroke.svg" />
                      </div>
                      <div className="column3">
                        <div className="arrow13">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath1.svg"
                          />
                        </div>
                        <div className="tooltip7">
                          <div className="content24">
                            <div className="text359">
                              <b className="tooltip-title3">Tooltip Title</b>
                              <div className="text360">
                                <p className="customers-with-a">{`Nam dapibus nisl vitae elit fringilla rutrum. Aenean sollicitudin, erat a elementum rutrum, neque sem pretium metus, quis mollis nisl nunc et massa. Vestibulum sed metus in lorem tristique ullamcorper id vitae erat. `}</p>
                                <p className="customers-with-a">&nbsp;</p>
                                <p className="customers-with-a">
                                  Nulla mollis sapien sollicitudin lacinia
                                  lacinia.
                                </p>
                              </div>
                            </div>
                            <div className="iconbutton">
                              <div className="iconbox">
                                <div className="icon187">
                                  <img
                                    className="icon188"
                                    alt=""
                                    src="/icon21.svg"
                                  />
                                </div>
                              </div>
                            </div>
                          </div>
                          <div className="slot2">
                            <div className="slot3">Slot</div>
                          </div>
                        </div>
                        <div className="arrow14">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrow1.svg"
                          />
                          <img
                            className="stroke-icon1"
                            alt=""
                            src="/stroke3.svg"
                          />
                        </div>
                      </div>
                      <div className="arrow15">
                        <img className="arrow-icon" alt="" src="/arrow2.svg" />
                        <img
                          className="stroke-icon"
                          alt=""
                          src="/stroke2.svg"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
              <div className="cell361">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon171.svg" />
                  </div>
                </div>
              </div>
              <div className="cell362">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
              <div className="cell363">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
            </div>
            <div className="featurerow17">
              <div className="featurecell17">
                <div className="content16">
                  <div className="feature-name3">{`PCI DSS 4.0 `}</div>
                  <div className="description67">
                    Azion might give early access to new features for customers
                    under Developer Plan that are still under refinement by our
                    product teams. It’s important to note that these features
                    are not covered by a Service Level Agreement.
                  </div>
                </div>
                <div className="infotooltip3">
                  <div className="infotooltip-main">
                    <div className="icon135">
                      <img className="icon136" alt="" src="/icon20.svg" />
                    </div>
                    <div className="tooltip">
                      <div className="arrow12">
                        <img className="arrow-icon" alt="" src="/arrow.svg" />
                        <img className="stroke-icon" alt="" src="/stroke.svg" />
                      </div>
                      <div className="column3">
                        <div className="arrow13">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath1.svg"
                          />
                        </div>
                        <div className="tooltip7">
                          <div className="content24">
                            <div className="text359">
                              <b className="tooltip-title3">Tooltip Title</b>
                              <div className="text360">
                                <p className="customers-with-a">{`Nam dapibus nisl vitae elit fringilla rutrum. Aenean sollicitudin, erat a elementum rutrum, neque sem pretium metus, quis mollis nisl nunc et massa. Vestibulum sed metus in lorem tristique ullamcorper id vitae erat. `}</p>
                                <p className="customers-with-a">&nbsp;</p>
                                <p className="customers-with-a">
                                  Nulla mollis sapien sollicitudin lacinia
                                  lacinia.
                                </p>
                              </div>
                            </div>
                            <div className="iconbutton">
                              <div className="iconbox">
                                <div className="icon187">
                                  <img
                                    className="icon188"
                                    alt=""
                                    src="/icon21.svg"
                                  />
                                </div>
                              </div>
                            </div>
                          </div>
                          <div className="slot2">
                            <div className="slot3">Slot</div>
                          </div>
                        </div>
                        <div className="arrow14">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrow1.svg"
                          />
                          <img
                            className="stroke-icon1"
                            alt=""
                            src="/stroke3.svg"
                          />
                        </div>
                      </div>
                      <div className="arrow15">
                        <img className="arrow-icon" alt="" src="/arrow2.svg" />
                        <img
                          className="stroke-icon"
                          alt=""
                          src="/stroke2.svg"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cell304">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell365">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon131.svg" />
                  </div>
                </div>
              </div>
              <div className="cell365">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
              <div className="cell365">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
            </div>
            <div className="featurerow17">
              <div className="featurecell3">
                <div className="content16">
                  <div className="feature-name3">SOC 2 Type 2 (and SOC 3)</div>
                  <div className="description67">Federated authentication</div>
                </div>
                <div className="infotooltip3">
                  <div className="infotooltip-main">
                    <div className="icon135">
                      <img className="icon136" alt="" src="/icon20.svg" />
                    </div>
                    <div className="tooltip">
                      <div className="arrow12">
                        <img className="arrow-icon" alt="" src="/arrow.svg" />
                        <img className="stroke-icon" alt="" src="/stroke.svg" />
                      </div>
                      <div className="column3">
                        <div className="arrow13">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrowpath1.svg"
                          />
                        </div>
                        <div className="tooltip7">
                          <div className="content24">
                            <div className="text359">
                              <b className="tooltip-title3">Tooltip Title</b>
                              <div className="text360">
                                <p className="customers-with-a">{`Nam dapibus nisl vitae elit fringilla rutrum. Aenean sollicitudin, erat a elementum rutrum, neque sem pretium metus, quis mollis nisl nunc et massa. Vestibulum sed metus in lorem tristique ullamcorper id vitae erat. `}</p>
                                <p className="customers-with-a">&nbsp;</p>
                                <p className="customers-with-a">
                                  Nulla mollis sapien sollicitudin lacinia
                                  lacinia.
                                </p>
                              </div>
                            </div>
                            <div className="iconbutton">
                              <div className="iconbox">
                                <div className="icon187">
                                  <img
                                    className="icon188"
                                    alt=""
                                    src="/icon21.svg"
                                  />
                                </div>
                              </div>
                            </div>
                          </div>
                          <div className="slot2">
                            <div className="slot3">Slot</div>
                          </div>
                        </div>
                        <div className="arrow14">
                          <img
                            className="arrow-icon"
                            alt=""
                            src="/arrow1.svg"
                          />
                          <img
                            className="stroke-icon1"
                            alt=""
                            src="/stroke3.svg"
                          />
                        </div>
                      </div>
                      <div className="arrow15">
                        <img className="arrow-icon" alt="" src="/arrow2.svg" />
                        <img
                          className="stroke-icon"
                          alt=""
                          src="/stroke2.svg"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cell296">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon141.svg" />
                  </div>
                </div>
              </div>
              <div className="cell297">
                <div className="iconbox">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon131.svg" />
                  </div>
                </div>
              </div>
              <div className="cell298">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
              <div className="cell299">
                <div className="icon117">
                  <div className="check-circle">
                    <img className="icon106" alt="" src="/icon121.svg" />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default PlansPage;
